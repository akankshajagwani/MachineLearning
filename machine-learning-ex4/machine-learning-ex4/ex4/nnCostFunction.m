...
%         computed in ex4.m
        ...
        for i = 1:m
           ...
           J = J + (-y1'*log(h_theta_x(:,i)))  - (1-y1)'*log(1-h_theta_x(:,i));
        end
        ...
        Reg =lambda/(2*m)*( sum(Theta11.*Theta11) + sum(Theta22.*Theta22));
        J = J+Reg;
...
%               first time.
        ...
        for t = 1:m
            ...
            err_term3 = h_theta_x - y1;
            err_term2 = transpose(Theta2(:,2:end))*err_term3 .* sigmoidGradient(z2);
            
            delta_1 = delta_1 + err_term2*transpose(a1);
            delta_2 = delta_2 + err_term3*transpose(a2);
